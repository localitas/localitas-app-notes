...
```bash
localitas-core app-store add --name notes --compose ./docker-compose.yml --port 9202
localitas-core app-store start notes
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-notes:latest`. To publish a new version:
